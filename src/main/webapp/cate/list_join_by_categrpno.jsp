...
<TABLE class='table table-striped'>
  ...
  <c:forEach var="categrp_Cate_join" items="${list }">  <!-- request 객체에 접근 -->
    ...
    <TR>
      ...
      <TD class="td_bs_left"><A href="./read_update.do?cateno=${cateno }&categrpno=${categrpno}">${name }</A></TD>
      <TD class="td_bs">${rdate.substring(0, 10) }</TD>
      <TD class="td_bs">
        <c:choose>
          <c:when test="${visible == 'Y'}">
            <A href="./update_visible.do?cateno=${cateno }&visible=${visible }&categrpno=${categrpno}"><IMG src="./images/open.png"></A>
          </c:when>
          <c:otherwise>
            <A href="./update_visible.do?cateno=${cateno }&visible=${visible }&categrpno=${categrpno}"><IMG src="./images/close.png"></A>
          </c:otherwise>
        </c:choose>
      </TD>
      ...
      <TD class="td_bs">
        ...
        <A href="./update_seqno_up.do?cateno=${cateno }&categrpno=${categrpno}"><span class="glyphicon glyphicon-arrow-up"></span></A>
        <A href="./update_seqno_down.do?cateno=${cateno }&categrpno=${categrpno}"><span class="glyphicon glyphicon-arrow-down"></span></A>         
      </TD>             
    </TR>
  </c:forEach>
  </tbody>
